...
## Anti-Bot / CAPTCHA Sites

Some websites actively block automated extraction with Cloudflare, CAPTCHAs, or other anti-bot protections (e.g. Trustpilot, Glassdoor, Yelp, some social media platforms). When `tavily_extract` or `tavily_crawl` returns:

- an image instead of text
- a login / CAPTCHA / verification page
- truncated or empty content for a site known to have protections

**Do not retry, do not switch URLs, do not alternate between search and extract.** Instead:

1. Try a focused `tavily_search` with `includeRawContent: true` to see if snippets contain useful info.
2. If snippets are insufficient, inform the user that the site blocks automated access and offer alternatives (e.g., search for cached copies, news articles summarizing the content, or suggest the user visit the site directly).
3. Move on — wasting multiple attempts on a protected site provides no value.

This applies to sites that return images, login walls, or "Verify you are human" pages. The goal is efficiency, not brute-forcing protected content.
